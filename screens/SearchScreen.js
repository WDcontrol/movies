...
import { Text, View, StyleSheet, TextInput, FlatList, ScrollView, Button } from 'react-native';
...
class SearchScreen extends React.Component{
  state={
    ...
    componentDidMount(){
      //Comédies
      this.props.tmdbService.getMoviesByCategories(35).then((res)=>{
          this.setState({comedies: res.data})
      }).catch((err)=>console.log(err))

      //Action
      this.props.tmdbService.getMoviesByCategories(28).then((res)=>{
        this.setState({action: res.data})
    }).catch((err)=>console.log(err))

      //Documentary
      this.props.tmdbService.getMoviesByCategories(99).then((res)=>{
        this.setState({documentaries: res.data})
    }).catch((err)=>console.log(err))

      //SF
      this.props.tmdbService.getMoviesByCategories(10749).then((res)=>{
    ...
    render(){      
        return(
            ...
            <ScrollViewComponent movies={this.state.comedies}></ScrollViewComponent>

            <Text style={styles.categories}>Action :</Text>
            <ScrollViewComponent movies={this.state.action}></ScrollViewComponent>

            <Text style={styles.categories}>Documentaires :</Text>
            <ScrollViewComponent movies={this.state.documentaries}></ScrollViewComponent>
            
            <Text style={styles.categories}>Romance :</Text>
            <ScrollViewComponent movies={this.state.romance}></ScrollViewComponent>
